...
export default function ShoppingCart() {
  ...
  function ModifyNumber(i: number, e: ItemType, condition: boolean) {
    ...
    if (newNumberArray[e.id] === 0) {
      ...
      newArray.splice(i, 1);
      setitems([...newArray]);
      SetDataToStorage(newArray, newNumberArray);
    }
    setItemsNumber([...newNumberArray]);
  }
        ...
        <section className="shoppingCart">
          <main className="shoppingItems">
            {items.map((e: ItemType, i: number) => {
              return (
                <article className="item" key={e.id}>
                  <section className="itemInfo">
                    <h1>{e.name}</h1>
                    <img src={e.src}></img>
                    <span>
                      <small>
                        {e.price} * {itemsNumber[e.id]}{" "}
                      </small>
                  ...
                  <section className="icons">
                    <FontAwesomeIcon
                      icon={faX}
                      className="xIcon"
                      onClick={() => DeleteItem(e, i)}
                    />
                    <span className="downIcons">
                      <FontAwesomeIcon
                        icon={faPlus}
                        className="plusIcon"
                        onClick={() => ModifyNumber(i, e, true)}
                      />
                      <FontAwesomeIcon
                        icon={faMinus}
                        className="minusIcon"
                        onClick={() => ModifyNumber(i, e, false)}
                      />
                    </span>
                  </section>
                </article>
